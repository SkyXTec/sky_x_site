...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const verTodasMsg = document.querySelector('.last-messages .section-link[data-view="mensagens"]');
    if (verTodasMsg) {
        verTodasMsg.addEventListener('click', function(e) {
            e.preventDefault();
            const navMensagens = document.querySelector('.sidebar-nav .nav-item[data-view="mensagens"]');
...
function initNavigation() {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            const targetView = document.querySelector(`.dash-view[data-view="${target}"]`);
            if (targetView) targetView.classList.add('active');

            // Atualiza título e hash
            if (pageTitle) pageTitle.textContent = this.getAttribute('data-label') || target;
...
function initActiveViewFromHash() {
    ...
    const link = document.querySelector(`.sidebar-nav .nav-item[data-view="${hash}"]`);
    if (link) link.click();
}
...
function renderMessages(tab) {
    ...
    if (!template) {
            list.innerHTML = '<p class="msg-template-error">Erro: Template de mensagem não encontrado no HTML.</p>';
        return;
    }
    filtered.forEach(msg => {
        ...
        if (dateEl) {
            let msgDt;
            if (typeof msg.data === 'string' && msg.data.match(/^\d{4}-\d{2}-\d{2}T/)) {
            ...
            const msgDay = new Date(msgDt.getFullYear(), msgDt.getMonth(), msgDt.getDate());
            const diffDays = Math.round((today - msgDay) / (24 * 60 * 60 * 1000));
            const hora = msgDt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            if (diffDays === 0) {
        ...
        if (btnAnswered) btnAnswered.onclick = () => { console.log('Botão: answered', msg.id); markAs('answered', msg.id); };
        if (btnArchived) btnArchived.onclick = () => { console.log('Botão: archived', msg.id); markAs('archived', msg.id); };
        if (btnUnarchive) btnUnarchive.onclick = () => { console.log('Botão: unarchive', msg.id); markAs('answered', msg.id); };
        if (btnMarkUnread) btnMarkUnread.onclick = () => { console.log('Botão: unread', msg.id); markAs('unread', msg.id); };
...
window.markAs = async function(status, id) {
    ...
    try {
        ...
        if (!data.success) {
            alert('Erro ao atualizar mensagem: ' + (data.error || 'Erro desconhecido.'));
            return;
        }
        msg.status = status;
...
function updateOverviewMessages() {
    ...
    const metricLabel = Array.from(document.querySelectorAll('.metric-label')).find(el => el.textContent.includes('Mensagens Novas'));
    if (metricLabel) {
    ...
    const grid = document.querySelector('.dash-view[data-view="overview"] .messages-grid');
    if (grid) {
        ...
        sorted.slice(0, 3).forEach(msg => {
            ...
            card.className = 'message-card' + (msg.status === 'unread' ? ' unread' : '');
            // Formatação da data
            let msgDate = '';
            if (msg.data) {
                let msgDt;
