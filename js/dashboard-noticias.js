...
function openNoticiaModal(noticia = null) {
    ...
    if (noticia) {
        ...
        tagsSelecionadas = noticia.tags ? noticia.tags.split(',').map(t => t.trim()).filter(Boolean) : [];
        if (noticia.capa_url) {
            if (/^\d+$/.test(noticia.capa_url)) {
                imgPreview.src = 'api/get-imagem.php?id=' + noticia.capa_url;
                imgPreview.setAttribute('data-capa-url', 'api/get-imagem.php?id=' + noticia.capa_url);
            } else {
                imgPreview.src = noticia.capa_url.startsWith('/') ? noticia.capa_url : '/' + noticia.capa_url;
...
async function renderNoticiasList() {
    ...
    try {
        ...
        if (noticias.length === 0) {
            list.innerHTML = '<span style="color:#888;">Nenhuma notícia cadastrada.</span>';
            return;
        }
                list.innerHTML = '';
                noticias.forEach(noticia => {
                        ...
                        `;
                        card.querySelector('.noticia-btn-editar').onclick = () => openNoticiaModal(noticia);
                        card.querySelector('.noticia-btn-pub').onclick = () => updateStatus(noticia.id, 'publicada');
                        card.querySelector('.noticia-btn-arq').onclick = () => updateStatus(noticia.id, 'arquivada');
                        list.appendChild(card);
                });
    // Função para atualizar status de publicação/arquivamento
    async function updateStatus(id, status) {
        ...
        list.innerHTML = '<span style="color:red;">Erro ao carregar notícias.</span>';
    }
}
...
async function saveNoticia(e) {
    ...
    const data = {
        ...
        tags,
        destaque,
        status: noticiaEditando && noticiaEditando.status ? noticiaEditando.status : 'publicada'
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    handleTagsInput();
    document.getElementById('noticiaImagem').addEventListener('change', function(e) {
